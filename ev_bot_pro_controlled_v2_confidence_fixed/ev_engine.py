...
def debug_all_sports():
    ...
    for label, key in SPORTS.items():
        try:
            ...
            result["sports"][label] = {"ok":True,"sport_key":key,"games_count":len(games),"from_cache":from_cache,"cache_seconds_left":ttl_left,"sample_games":[{"home_team":g.get("home_team"),"away_team":g.get("away_team"),"commence_time":g.get("commence_time"),"bookmakers_count":len(g.get("bookmakers", []))} for g in games[:5]]}
        except Exception as e:
