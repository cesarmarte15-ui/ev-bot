...
def get_dashboard(selected_sports: list, force_refresh: bool = False) -> dict:
    ...
    # --- Soccer value via API-Football ---
    try:
        football_picks = get_football_value_picks(force_refresh=force_refresh)
        total_football = len(football_picks)
        dash["sports"]["Soccer (API-Football)"] = {
            "ok": True, "sport_key": "api-football",
            "games_count": total_football, "total_api_games": total_football,
            "from_cache": False, "cache_seconds_left": CACHE_TTL,
        }
        for sig in football_picks:
            if sig.get("color") == "gold":
                dash["value_gold"].append(sig)
            elif sig.get("color") == "silver":
                dash["value_silver"].append(sig)
    except Exception as e:
        logger.warning("API-Football error: %s", e)
        dash["sports"]["Soccer (API-Football)"] = {
            "ok": False, "sport_key": "api-football", "error": str(e)
        }
        dash["warnings"].append(f"Soccer: {e}")
...
# Liga ID -> nombre (los más importantes con value)
FOOTBALL_LEAGUES: dict[int, str] = {
    39:  "Premier League",
    140: "La Liga",
    135: "Serie A",
    78:  "Bundesliga",
...
def fetch_football_odds(league_id: int, force_refresh: bool = False):
    """Obtiene odds de API-Football para una liga."""
    if not FOOTBALL_API_KEY:
        raise RuntimeError("Falta FOOTBALL_API_KEY")

    key = f"football_odds_{league_id}"
    if not force_refresh:
...
def get_football_value_picks(force_refresh: bool = False) -> list[dict]:
    ...
    for league_id, league_name in FOOTBALL_LEAGUES.items():
        try:
            games, _ = fetch_football_odds(league_id, force_refresh)
            for g in games:
